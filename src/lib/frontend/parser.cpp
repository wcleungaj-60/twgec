...
bool Parser::parseActionArgs(std::unique_ptr<ActionNode> &actionNode,
  ...
  if (!consume(TokenType::IDENTIFIER, /*errorThrowing*/ false)) {
    ...
    if (!consume(TokenType::STRING))
      return false;
    std::unique_ptr<PositionalArgNode> posArgNode = std::make_unique<PositionalArgNode>(firstToken, loc);
    ...
    if (!consume(TokenType::STRING))
      return false;
    std::unique_ptr<NamedArgNode> namedArgNode = std::make_unique<NamedArgNode>(firstToken, secondToken, loc);
...
std::unique_ptr<TriggersNode> Parser::parseTriggers() {
  ...
  if (!consume(TokenType::TRIGGERS) || !consume(TokenType::OPENCUR) ||
      !consume(TokenType::CLOSECUR))
    return nullptr;
  std::unique_ptr<TriggersNode> triggersNode = std::make_unique<TriggersNode>(loc);
  return triggersNode;
}