...
public:
  Location loc;
  std::string value;
  PositionalArgNode(const std::string &value, Location loc) : value(value), loc(loc) {}
};
...
public:
  ...
  std::string key;
  std::string value;
  NamedArgNode(const std::string &key, const std::string &value, Location loc) : key(key), value(value), loc(loc) {}
};
